...
class CodeBreakerGame:
    ...
    def save_game(self):
        ...
        while True:
            ...
            if choice == 'c':
                print("Save cancelled. Returning to the game.")
                return
    ...
    def load_game(self):
        ...
        while True:
            choice = input("What save would you like to load (1, 2, ..., or 'c' to cancel): ").strip().lower()
            if choice == 'c':
                print("cancelled")
                return None, None, None
            ...
            if choice.isdigit() and 1 <= int(choice) <= len(save_slots):
                ...
                if filename:
                    try:
                        ...
                        return data['solution'], data['attempts_left'], data['history']
...
class SaveManager:
    ...
    def list_save_files():
        ...
        slots = []
        file_map = {}
        for i in range(1, 4):  # Ensure slots 1, 2, 3 exist
            filename = f"save_slot_{i}.json"
            if filename in save_files:  
                with open(filename, "r") as file:
                    data = json.load(file)
                display_name = f"{data['name']} - {data['date']}"
                slots.append(display_name)
                file_map[display_name] = filename
            else:
                slots.append("empty")
                file_map[f"empty_{i}"] = filename  # For empty slots, associate with filename
...
# def display_board(solution_length, history):
#     print("--------------------------------------------------------------------------")
#     print("   ==================+=====")  
...
#     # Print guesses and feedback in the correct order, most recent first
#     for guess, feedback in reversed(history):  # Display the most recent guess first
...
def display_board(solution, history):
    print("--------------------------------------------------------------------------")
    print("   ==================+=====")  
    print("    o  o  o  o  o  o | R W  ")  
    print("   ==================+=====")  

    # If there is a winning guess, display it at the top row
    ...
    for _ in range(10 - len(history)-1):  
        print("    " + "  ".join("o" for _ in range(6)) + " | 0 0  ")  

    # Print guesses from bottom to top (most recent guesses at the bottom)
...
def game_loop(game):
    while game.attempts_left > 0:
        ...
        guess = input("What is your guess (q to quit, s to save and quit): ").strip().lower()

        if guess == 'q':
            print("Ending Game.")
            break  # Exit game when quit
        ...
        valid, message = validate_guess(guess, len(game.solution))
        if not valid:
            print(f"Your guess was \"{guess}\". {message}")
            continue
    ...
    if game.attempts_left == 0:
        ...
        print(f"The correct code was: {game.solution}")
